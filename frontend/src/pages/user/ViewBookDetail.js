...
import BorrowModal from "../../components/BorrowModal"; // Đảm bảo đường dẫn đúng
...
const ViewBookDetail = () => {
  ...
  const handleModalConfirm = async ({ quantity, isReadOnSite, returnDate }) => {
    setLoading(true);
    try {
      for (let i = 0; i < quantity; i++) {
        await requestBorrowBook({
          bookId: book._id,
          isReadOnSite,
          notes: `Trả trước ngày ${returnDate}`,
        });
      }
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <p>
              <strong>Thể loại:</strong> {book.category || "Không xác định"}
            </p>
            <p>
